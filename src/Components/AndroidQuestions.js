...
import { ANDROID_QUESTIONS } from "../Constants";
...
class AndroidQuestions extends Component {
	initialState = {
		...
		totalQuestions: ANDROID_QUESTIONS.length,
	...
	//handle text input (user answer field) change
	_handleInputChange = (event) => {
		this.setState({
			answerFieldValue: event.target.value,
		});
	};

	render() {
		const questionString = ANDROID_QUESTIONS[this.state.questionIndex].question;
		const answerString = ANDROID_QUESTIONS[this.state.questionIndex].answer;
				...
				{!setComplete && (
					<div>
						...
						<div className="flex justify-center mt3 ml5 mr5 ph2">
							<div className="bg-silver br2 w-70 ph2">
								<TextField
